...
class ViewController: UIViewController,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        print(dhInstance.dateFormatter(dateString: Date(), formatterString: "dd-MMM-YYYY"))
        print(dhInstance.dateFormatter(dateString: Date(), formatterString: "dd MM YYYY HH:mm"))
        print(dhInstance.dateFormatter(dateString: Date(), formatterString: "dd/MM/YYYY"))
        print(dhInstance.dateFormatter(dateString: Date(), formatterString: "dd YYY"))
        print(dhInstance.dateFormatter(dateString: Date(), formatterString: "dd MM YYYY hh:mm"))
       
        
        print("#########################################TIME AGO SINCE DATE#################################")
        let twentyFiveYearsdate = Date.init(timeIntervalSinceReferenceDate: -5000000*60)
        print(dhInstance.timeAgoSinceDate(twentyFiveYearsdate, currentDate: Date(), numericDates: true))
        
        let sixteenYearsAgo = Date.init(timeIntervalSinceReferenceDate: -50000*60)
        print(dhInstance.timeAgoSinceDate(sixteenYearsAgo, currentDate: Date(), numericDates: true))
        
        print(dhInstance.timeAgoSinceDate(Date(), currentDate: Date(), numericDates: true))
    ...
    @IBAction func validateEmailBtnTapped(_ sender: Any) {
        ...
        let isValid = dhInstance.isValidEmail(testStr: self.emailTextField.text!)
